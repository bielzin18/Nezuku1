...
mett = ["1", "2"]
res = mett[Math.floor(Math.random() * mett.length)]
teks = `O integrante @${num.split('@')[0]} corno 🐂 saiu do grupo... bye bye 👋🏻 ainda foste tarde👎🏻👎🏻`
let buff = await getBuffer(ppimg)
tobi.sendMessage(mdata.id, buff, MessageType.image, {thumbnail:null, caption: teks})
}
}
...
/************** LINGUAGEM GRUPO ************/
  ...
  const ownerNumber = [`559291272406@s.whatsapp.net`] 
...
const ff = {
text:  `Sem flood @${sender.split('@')[0]}.......\n\nAguarde 7 segundos antes de usar outro comando✅`,
contextInfo: {
mentionedJid: [sender]
}
}
...
case 'oni':
d = []
teks = `Há um *${comando}* 😨 entre nós, extermine esse gay o mais breve possível🤣, antes que ele(a) 😈 o devore 👉:`
for(i = 0; i < 1; i++) {
r = Math.floor(Math.random() * groupMetadata.participants.length + 0)
teks += `@${groupMembers[r].jid.split('@')[0]}\n`
d.push(groupMembers[r].jid)
...
case 'muzan':
d = []
teks = `Acabei de Rastrear o *${comando}*🕴 pelo cheiro, cuidado 🤔 pois ele pode ir até você e usar poderosas técnicas de Mestre Onin, e Magia de outro Nível. Ouvi dizer que ele já matou centenas de caçadores 🥷🏼🥷🏼🥷🏼 por mais de séculos, Mate-o se puder  👀  👉:`
for(i = 0; i < 1; i++) {
r = Math.floor(Math.random() * groupMetadata.participants.length + 0)
teks += `@${groupMembers[r].jid.split('@')[0]}\n`
d.push(groupMembers[r].jid)
...
　　.　　　.　　　  　　.　　　　　。　　   。　.
　.　　      。　        ඞ   。　    .    •
•   @${mentioned[0].split('@')[0]} 🚀 E J E T A D O 🚀
                  -1 impostor oni fdp 🤬 。　.
　 　　。　　 　　　　ﾟ　　　.　      　　　.
,　　　　.                  .`
...
addFilter(from)
if (isLimit(sender)) return webPage(linguagem.cash_adquirir(pushname, p), {quoted: mek}) 
const tiro = ["pow","atirou","pow","pow","atirou","pow","atirou","atirou","pow","pow"]
const figr = ["pattta1","pattta2","pattta3"]
tpa = tiro[Math.floor(Math.random() * (tiro.length))]	
tpb = figr[Math.floor(Math.random() * (figr.length))]
figb = fs.readFileSync('./base de dados/database/figurinhas/'+tpb+'.webp')
if (tpa == "atirou") {
var morte = "Você teve sorte dessa vez, a Pistola não atirou."
} else if (tpa == "pow") {
var morte = "Tinha uma bala na Pistola, POW!"
}
if (morte == "Tinha uma bala na Pistola, POW!") {
setTimeout( () => {
...
cararo = ["cara" , "coroa" , "cara" , "coroa" , "coroa" , "cara" , "coroa"]
...
case 'nezuku': case 'tanjiro': case 'zenitsu': case 'inosuke':
case 'tomioka': case 'shinobu': case 'urokodaki': case 'rengoku':
case 'kibutsuji': case 'gyomei': case 'sabito': case 'makomo':
case 'akaza': case 'tengen': case 'obanai': case 'mitsuri': 
case 'sanemi': case 'muichiro': case 'haganezuka':
addFilter(from)
if (isLimit(sender)) return webPage(linguagem.cash_adquirir(pushname, p), {quoted: mek})
enviar(mess.espere)
teks = `${comando}`
...
payout = q
const koinPerlimit = 3000
var total3 = koinPerlimit * payout
if ( checkATMuser(sender) <= tota6) return enviar(`Desculpe, você não tem dinheiro suficiente. por favor colete e compre mais tarde`)
...
addFilter(from)
if (isLimit(sender)) return webPage(linguagem.cash_adquirir(pushname, p), {quoted: mek})
if (!isOwner) return enviar("Só meu dono @biel")
...
if (Number(args[0]) >= checkxpr || Number(args[0]) >= dinheiro) return enviar(`Você não pode apostar uma quantidade de dinheiro maior do que você tem, e nosso limite de apostas é de ${quantidader} dinheiro por vez!\n\nSeu dinheiro: ${checkxpr}`)
...
if (double == 1) {
await enviar(`🗡️TRIM!!!\n\nSua espada quebrou 🗡️, causando uma perca de ${nrolxp} para ⚙️consertá-la⚙️⚒️.`)
addKoinUser(sender, nrolxp, dinheiro)
addKoinUser(`${OwnerNumber}@s.whatsapp.net`, nrolxp)
} else if (double == 2) {
await enviar(`🙌🏻Sobreviveu😇\n\nVocê conseguiu 🪓 Matar onins poderosos 🛡️⚔️👹 e sobreviveu, ganhando uma recompensa de ${prolxp} em dinheiro 💵 `)
addKoinUser(sender, prolxp, dinheiro)
} else if (double == 3) {
await enviar(`👎🏼QUE MÁ SORTE😔\n\nPerdeu ${nrolxp} 👹 Onins‍ entraram em sua casa🏯, e comeram seus órgãos 🫀 enquanto dormia 😴☠️.`)
addKoinUser(sender, nrolxp, dinheiro)
addKoinUser(`${OwnerNumber}@s.whatsapp.net`, nrolxp)
} else if (double == 4) {
await enviar(`😑 MEDROSO 😑 \n\nVocê pulou o muro🏃🏼‍♂️ 🧗🏻‍♂️, e se salvo dos Onin 😨.\nPerdeu ${nrolxp} em Dinheiro 💰, por ser um covarde e deixa pessoas morrerem. 😏 domine as fase de 😮‍💨 respirações e mate oni como um 🥷🏼 caçador de verdade.`)
addKoinUser(sender, nrolxp, dinheiro)
} else if (double == 5) {
await enviar(`💀 E MORREU - QUE PENA ⚰️\n\nAcabou perdendo ${nrolxp} em seu dinheiro, pois um 👀 👹 onim habilidoso o matou em batalha 🪖⚰️.`)
addKoinUser(sender, nrolxp, dinheiro)
addKoinUser(`${OwnerNumber}@s.whatsapp.net`, nrolxp)
} else if (double == 6) {
await enviar(`✨IMPOSSÍVEL💫\n\nVocê finalmente matou um onim 🌙 lua inferior 🥷🏼🏹👹 receba seus ${prolxp} em dinheiro 💰, prossiga assim e logo se tornará um 🔥️Hashira🔥.`)
addKoinUser(sender, prolxp, dinheiro)
await requestCash(sender)
}
break
...
case 'informes':
if (!isOwner) return enviar("Somente meu dono lek")
